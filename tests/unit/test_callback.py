...
class TestAsyncPreCallHook:
    async def test_blocks_when_rule_matches(self, logger):
        import litellm
        logger.governance._rules = [{
            "_id": "rule-async-1",
            "type": "regex",
    ...
    async def test_passes_when_no_rules(self, logger):
        logger.governance._rules = []
        logger.governance._fetched_once = True
        data = {"model": "gemini-flash", "messages": []}
    ...
    async def test_passes_when_rule_does_not_match(self, logger):
        logger.governance._rules = [{
            "_id": "rule-async-2",
            "type": "regex",
            "block": True,
    ...
    async def test_blocked_event_sent_to_moesif(self, logger):
        logger.governance._rules = [{
            "_id": "rule-async-3",
            "type": "regex",
            "block": True,
        ...
        with respx.mock:
            route = respx.post("https://api.moesif.net/v1/events/batch").mock(return_value=httpx.Response(201))
